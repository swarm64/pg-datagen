...
class Column:
    ...
    @property
    def do_skip(self):
        """Skip column during generation."""
        return self.gen == 'skip'

    @property
    def is_linked(self):
        """Determine whether this column needs to be generated by choosing from list."""
        return any((
            self.gen.startswith('choose_from_list'),
            self.gen.startswith('choose_from_static_list')))

    def generate(self, rand_gen: Type[Random]) -> Any:
        """
        ...
        """
        if self.none_prob:
            if rand_gen.bool_sample(self.none_prob):
                return None

        if self.is_linked:
            return None

        return getattr(rand_gen, self.gen)(*self.args)
